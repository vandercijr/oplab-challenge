...
jQuery(document).ready(function ($) {
  var $lateral_menu_trigger = $('.nav-trigger'),
      $content_wrapper = $('.main'); //open-close lateral menu clicking on the menu icon
  ...
  $lateral_menu_trigger.on('click', function (event) {
    ...
    $('#ar-about').toggleClass('is-active'); //check if transitions are not supported - i.e. in IE9
  ...
  $content_wrapper.on('mouseover', function (event) {
    if (!$(event.target).is('.nav-trigger, .nav-trigger span')) {
      ...
      $('#ar-about').removeClass('is-active'); //check if transitions are not supported
